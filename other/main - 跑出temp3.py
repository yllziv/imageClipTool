...
while row:
    ...
    if not int(row.getValue("OBJECTID")) in over.overArray: #若元素不在数组中
        if(mycount == 1 or mycount ==2):
            mycount = mycount+1
            row = cursor.next()
            continue
        # print "当前切割的影像类别为: " + str(row.getValue(field))
        # print "Start Clip classFile"
        print  str(classResultPath)+str(row.getValue(field))+"/"+str(classType[indexOfClass])+".tif"
        ...
        #使用掩模
        clip(raster,rasterclass,indexOfClass+1,resultPath)
